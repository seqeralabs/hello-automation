"""Configuration settings for hello-automation.

Note: This module is kept for backwards compatibility. The Dagster implementation
uses ConfigurableResource classes that read directly from environment variables.
See resources.py and definitions.py for the Dagster configuration approach.
"""
...
class Settings(BaseSettings):
    """Configuration settings for hello-automation application."""

    # File processing settings
    sequencer_monitor_path: DirectoryPath = DirectoryPath("./sequencer")
    metadata_download_url: AnyHttpUrl = AnyHttpUrl(
    ...
    seqera_token: SecretStr = Field(alias="TOWER_ACCESS_TOKEN")
    seqera_workspace_id: str = Field(alias="TOWER_WORKSPACE_ID")
    ...
    studio_cpu: int = Field(
        default=2, description="Number of CPUs to allocate for the studio"
    )
    studio_memory: int = Field(
        default=8, description="Amount of memory in GB to allocate for the studio"
    )
    studio_conda_env: str = Field(
        default="base", description="Conda environment to use for the studio"
    )
